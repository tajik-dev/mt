...
private $pwd_hint=true;

public function pwd_hint(){return $this->pwd_hint;}
...
public function login($login='',$password=''){
	...
	if($login!='' && $password!=''){
		if($this->valid('login',$login) && $this->valid('pwd',$password)){
			$DB=\DB::init();
			if($DB->connect()){
...
public function valid($type='',$value=''){
	$valid=false; $len=strlen(trim($value));
	switch ($type) {
		case 'login':
			if($len>3 && $len<128){
...
public function passwd($pwd='',$uid=0) {
	...
	if($this->valid('pwd',$pwd)){
		if($uid==0) { $uid=(int) \CORE\USER::init()->get('uid'); }
		$DB=\DB::init();
		if($DB->connect()){
			...
			if(!$this->pwd_hint()) $pwd['hint']=NULL;
...
public function create($user='',$pwd='',$gid=0,$status=1,$pid=0){
	$valid=true; $errors='';
	// user data initialization
	...
	if($valid){
		...
		if($DB->connect()){
			...
			if($sth->rowCount()>0){
				...
				$sth->execute($usr);
				$DB->query_count();
				\CORE::init()->msg('info','New user successfully created.');
			}
		}
	} else {
